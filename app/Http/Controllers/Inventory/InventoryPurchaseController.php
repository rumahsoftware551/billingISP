...
class InventoryPurchaseController extends Controller {
    ...
    public function receive(Request $r,InventoryPurchaseOrder $purchase,InventoryLedgerService $ledger):RedirectResponse { $a=$this->manager($r); abort_unless((string)$purchase->tenant_id===app(CurrentTenant::class)->id(),404); $d=$r->validate(['purchase_item_id'=>'required|integer','quantity'=>'required|numeric|min:0.001','serials'=>'nullable|string|max:10000','notes'=>'nullable|string|max:2000']); DB::transaction(function()use($ledger,$purchase,$d,$a){$lockedPurchase=InventoryPurchaseOrder::query()->whereKey($purchase->id)->lockForUpdate()->firstOrFail();$item=$lockedPurchase->items()->whereKey($d['purchase_item_id'])->lockForUpdate()->firstOrFail();$remaining=(float)$item->quantity-(float)$item->received_quantity;abort_if((float)$d['quantity']>$remaining+0.0001,422,'Penerimaan melebihi sisa PO.');$sku=$item->sku()->firstOrFail();$assets=[];if($sku->serialized){foreach(preg_split('/\r\n|\r|\n/',trim((string)($d['serials']??''))) as $line){if(trim($line)==='')continue;[$sn,$mac,$barcode]=array_pad(array_map('trim',explode(',',$line,3)),3,'');$assets[]=['serial_number'=>$sn,'mac_address'=>$mac,'barcode'=>$barcode];}}$ledger->receive($lockedPurchase->destination,$sku,(float)$d['quantity'],(float)$item->unit_cost,$assets,$lockedPurchase->supplier_id,$lockedPurchase->id,$a->id,null,$d['notes']??'Penerimaan '.$lockedPurchase->po_number);$item->increment('received_quantity',(float)$d['quantity']);$total=(float)$lockedPurchase->items()->sum('quantity');$received=(float)$lockedPurchase->items()->sum('received_quantity');$lockedPurchase->update(['status'=>$received+0.0001>=$total?'received':'partial']);}); return back()->with('success','Penerimaan PO berhasil.'); }
}
