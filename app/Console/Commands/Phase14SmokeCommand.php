...
class Phase14SmokeCommand extends Command {
 ...
 public function handle():int {
  ...
  $demo=InventoryPortalAccount::query()->where('email','inventory@jaringanku.local')->first(); if(!$demo && !filter_var(env('SEED_DEMO_DATA',false),FILTER_VALIDATE_BOOL)){$this->warn('SEED_DEMO_DATA=false; Phase 14 smoke dilewati setelah preflight.');return self::SUCCESS;} if(!$demo || !$demo->passwordMatches(env('PHASE14_INVENTORY_PASSWORD','InventoryDemo123!'))){$this->error('Demo Inventory account/password belum valid.');return 2;}
