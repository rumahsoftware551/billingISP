...
export default function Layout({children}:{children:React.ReactNode}) {
  ...
  if(platformAdmin){groups.push({label:'Platform',items:[
  ...
  const initials=String(props.auth?.user?.name||props.auth?.user?.email||'U').split(/\s+/).slice(0,2).map((x:string)=>x.charAt(0)).join('').toUpperCase();
  ...
  return <div className="jk-shell min-h-screen" style={{'--jk-primary':primary} as React.CSSProperties}>
    ...
    <aside className={`jk-sidebar ${mobile?'translate-x-0':'-translate-x-full'} md:translate-x-0`}>
      <div className="flex h-18 items-center gap-3 border-b border-white/10 px-5">
        {branding.logo_url?<img src={branding.logo_url} className="h-9 w-9 rounded-lg bg-white p-1 object-contain"/>:<div className="grid h-9 w-9 place-items-center rounded-lg bg-white/10 font-black">J</div>}
        ...
        <button aria-label="Tutup menu" className="ml-auto rounded-lg p-2 text-slate-300 hover:bg-white/10 md:hidden" onClick={()=>setMobile(false)}><X size={19}/></button>
      </div>
      ...
      <nav className="jk-nav px-3 py-4">
        {groups.map(group=><div key={group.label} className="mb-4 last:mb-0">
          ...
          <div className="space-y-1">{group.items.map(item=>{const I=item.icon;return <Link key={item.href} href={item.href} onClick={()=>setMobile(false)} className={`jk-nav-link ${active(item.href)?'active':''}`}><I size={18}/><span>{item.name}</span></Link>})}</div>
        </div>)}
      </nav>
      ...
      <div className="mt-auto border-t border-white/10 p-4">
        ...
        <div className="mt-3 text-[10px] text-slate-500">v{props.release?.version||'1.3.0-dev'} · {props.release?.channel||'development'}</div>
      </div>
    </aside>

    {mobile&&<button aria-label="Tutup menu" className="fixed inset-0 z-30 bg-slate-950/45 backdrop-blur-[1px] md:hidden" onClick={()=>setMobile(false)}/>}

    <div className="jk-main md:ml-72">
      <header className="jk-topbar">
        <div className="flex min-w-0 items-center gap-3">
          <button aria-label="Buka menu" className="rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm md:hidden" onClick={()=>setMobile(true)}><Menu size={19}/></button>
          <div className="hidden sm:block"><div className="truncate text-sm font-bold text-slate-900">{props.tenant?.name||branding.company_name||'Jaringanku'}</div><div className="truncate text-xs text-slate-500">{props.access?.role?.name||'Administrator'}</div></div>
        </div>

        <div className="relative mx-auto hidden w-full max-w-xl lg:block">
          <Search size={16} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400"/>
          <input value={search} onChange={e=>setSearch(e.target.value)} className="jk-search pl-10" placeholder="Cari menu: pelanggan, billing, jaringan, inventory..."/>
          {results.length>0&&<div className="absolute top-12 z-50 w-full rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl">{results.map(r=><Link href={r.href} key={r.href} onClick={()=>setSearch('')} className="block rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">{r.name}</Link>)}</div>}
        </div>
