...
const input='mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-sky-500';
...
export default function IntegrationsIndex(props:any){
  ...
  return <Layout><Head title="Integrations"/><div className="space-y-6">
    <div><h2 className="text-2xl font-black">Payment & WhatsApp Integrations</h2><p className="text-sm text-slate-400 mt-1">Midtrans Snap/QRIS dan WhatsApp Cloud API per tenant. Secret disimpan terenkripsi.</p></div>
    ...
    <div className="grid xl:grid-cols-2 gap-6">
      <Card title="Payment Gateway">
        <form onSubmit={submitPayment} className="space-y-4">
          ...
          <div><div className="text-sm mb-2">Enabled payment methods</div><div className="grid grid-cols-2 md:grid-cols-3 gap-2">{methods.map(m=><label key={m} className="rounded border border-slate-800 px-2 py-2 text-xs flex gap-2"><input type="checkbox" checked={payment.data.enabled_payments.includes(m)} onChange={()=>toggleMethod(m)}/>{m}</label>)}</div></div>
      ...
      <Card title="WhatsApp Cloud API">
        <form onSubmit={submitWa} className="space-y-4">
          ...
          <div className="rounded-lg border border-slate-800 bg-slate-950 p-3 text-xs"><div className="text-slate-400">Webhook path</div><code className="break-all">{webhookPath}</code></div>
          <div><div className="text-sm mb-2">Approved template name mapping</div>{['billing.invoice_created','billing.overdue','billing.payment_received'].map(code=><div key={code} className="grid md:grid-cols-2 gap-2 items-center mb-2"><code className="text-xs text-slate-400">{code}</code><input className={input} value={(wa.data.template_map as any)?.[code]||''} onChange={e=>wa.setData('template_map',{...wa.data.template_map,[code]:e.target.value})} placeholder="Meta template name (optional)"/></div>)}</div>
        ...
        <form className="mt-5 border-t border-slate-800 pt-4 flex gap-2" onSubmit={e=>{e.preventDefault();test.post('/integrations/whatsapp/test',{preserveScroll:true});}}><input className={input+' mt-0'} value={test.data.recipient} onChange={e=>test.setData('recipient',e.target.value)} placeholder="0812..."/><button className="rounded-lg bg-emerald-700 px-4 text-sm font-semibold">Send test</button></form>
      </Card>
    </div>

    <Card title="Recent payment gateway transactions"><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="text-slate-400"><tr><Th>Order</Th><Th>Invoice</Th><Th>Provider</Th><Th>Amount</Th><Th>Status</Th><Th>Updated</Th></tr></thead><tbody>{(props.transactions||[]).map((t:any)=><tr key={t.id} className="border-t border-slate-800"><Td>{t.order_id}</Td><Td>{t.invoice?.invoice_number||'-'}</Td><Td>{t.provider}/{t.environment}</Td><Td>{rupiah(t.amount)}</Td><Td>{t.status}</Td><Td>{fmt(t.updated_at)}</Td></tr>)}{!(props.transactions||[]).length&&<tr><td colSpan={6} className="p-4 text-slate-500">Belum ada transaksi gateway.</td></tr>}</tbody></table></div></Card>
    <Card title="Recent WhatsApp delivery logs"><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="text-slate-400"><tr><Th>Recipient</Th><Th>Message ID</Th><Th>Status</Th><Th>Error</Th><Th>Updated</Th></tr></thead><tbody>{(props.whatsappLogs||[]).map((x:any)=><tr key={x.id} className="border-t border-slate-800"><Td>{x.recipient}</Td><Td>{x.provider_message_id||'-'}</Td><Td>{x.status}</Td><Td>{x.last_error||'-'}</Td><Td>{fmt(x.updated_at)}</Td></tr>)}{!(props.whatsappLogs||[]).length&&<tr><td colSpan={5} className="p-4 text-slate-500">Belum ada pesan WhatsApp.</td></tr>}</tbody></table></div></Card>
  </div></Layout>
}
function Card({title,children}:{title:string,children:React.ReactNode}){return <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-5"><h3 className="mb-4 text-lg font-bold">{title}</h3>{children}</section>}
function Label({title,children}:{title:string,children:React.ReactNode}){return <label className="block text-sm"><span className="text-slate-400">{title}</span>{children}</label>}
