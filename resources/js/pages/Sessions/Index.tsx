    ...
    <div className="space-y-6">
      ...
      <Surface title="Cari Session" subtitle="Cari berdasarkan PPPoE username, IP, MAC, service number, customer number, atau nama pelanggan.">
        <form onSubmit={submit} className="flex flex-col gap-3 md:flex-row">
          <div className="relative flex-1"><Search size={16} className="pointer-events-none absolute left-3 top-3 text-slate-500"/><input className={`${input} pl-9`} value={q} onChange={e=>setQ(e.target.value)} placeholder="Cari session..."/></div>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200">
          <table className="min-w-[1180px] w-full text-sm">
            ...
            <tbody className="bg-white">{sessions.data.map(s=><tr key={s.radacctid} className="border-t border-slate-100 align-top hover:bg-slate-50/60">
              <Td><div className="font-bold text-blue-700">{s.username}</div><Link href={`/customers/${s.customer_id}`} className="text-xs font-medium text-slate-700 hover:text-blue-700">{s.customer_number} · {s.customer_name}</Link><div className="text-[11px] text-slate-500">{s.service_number} · {s.service_status}</div></Td>
              <Td><div className="font-mono text-xs font-semibold text-slate-700">{s.framedipaddress||'-'}</div><div className="mt-1 text-xs text-slate-500">NAS {s.nas_shortname||s.nasipaddress||'-'}</div><div className="text-[11px] text-slate-500">Router {s.router_name||'-'} · MAC {s.callingstationid||'-'}</div></Td>
              <Td><div className="font-semibold text-slate-700">{s.plan_name||'-'}</div><div className="text-xs text-slate-500">{s.plan_code||''}</div></Td>
              <Td><div className="text-xs text-slate-700">{dt(s.acctstarttime)}</div><div className={`mt-1 inline-flex items-center gap-1 text-xs ${isStale(s)?'font-semibold text-amber-700':'text-slate-500'}`}>{isStale(s)&&<AlertTriangle size={11}/>}update {dt(s.acctupdatetime)}</div></Td>
              ...
              <Td><div className="font-semibold text-slate-700">{duration(s.acctsessiontime,s.acctstarttime)}</div><div className="mt-1 max-w-36 truncate font-mono text-[10px] text-slate-500" title={s.acctsessionid}>{s.acctsessionid}</div></Td>
