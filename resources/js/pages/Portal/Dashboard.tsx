    ...
    <div className="space-y-7">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          {services.map(s=><div key={s.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-start justify-between gap-3"><div><div className="font-bold">{s.service_number}</div><div className="mt-1 text-sm text-blue-700">{s.pppoe_username}</div></div><Status value={s.status}/></div>
            <div className="mt-4 grid grid-cols-2 gap-2 text-xs text-slate-500"><div>Paket<div className="mt-1 font-semibold text-slate-800">{s.plan?.name||'-'}</div></div><div>Kecepatan<div className="mt-1 font-semibold text-slate-800">{s.plan?`${Math.round(s.plan.download_kbps/1000)} / ${Math.round(s.plan.upload_kbps/1000)} Mbps`:'-'}</div></div><div>Harga<div className="mt-1 font-semibold text-slate-800">{rupiah(s.plan?.price||0)}</div></div><div>Terpasang<div className="mt-1 font-semibold text-slate-800">{date(s.installed_at)}</div></div></div>
          </div>)}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="space-y-2">
          {invoices.map(inv=><Link key={inv.id} href={`/portal/${portalTenantSlug}/invoices/${inv.id}`} className="flex flex-col gap-2 rounded-xl border border-slate-200 p-4 hover:border-sky-800 md:flex-row md:items-center md:justify-between">
            <div><div className="font-semibold text-blue-700">{inv.invoice_number}</div><div className="text-xs text-slate-500">{inv.service?.service_number||'-'} · terbit {date(inv.issued_at)} · jatuh tempo {date(inv.due_at)}</div></div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-4 overflow-x-auto"><table className="w-full text-sm"><thead className="text-left text-xs text-slate-500"><tr><th className="pb-2">Pembayaran</th><th>Tanggal</th><th>Metode</th><th>Jumlah</th><th></th></tr></thead><tbody>{payments.map(p=><tr key={p.id} className="border-t border-slate-200"><td className="py-3 font-semibold">{p.payment_number}</td><td>{date(p.paid_at)}</td><td className="uppercase">{p.method}</td><td>{rupiah(p.amount)}</td><td className="text-right"><a href={`/portal/${portalTenantSlug}/receipts/${p.id}/download`} className="text-blue-700 hover:underline">Receipt PDF</a></td></tr>)}</tbody></table>{payments.length===0&&<div className="py-4 text-sm text-slate-500">Belum ada pembayaran.</div>}</div>
      </section>
    </div>
  </PortalLayout>;
}
function Metric({label,value,accent=false}:{label:string,value:any,accent?:boolean}){return <div className={`rounded-xl border p-4 ${accent?'border-amber-200 bg-amber-50':'border-slate-200 bg-white'}`}><div className="text-xs text-slate-500">{label}</div><div className="mt-2 text-xl font-black">{value}</div></div>}
function Status({value}:{value:string}){const v=(value||'').toLowerCase();const cls=['active','paid','posted'].includes(v)?'bg-emerald-50 text-emerald-200 border-emerald-200':['overdue','suspended','partial'].includes(v)?'bg-amber-50 text-amber-700 border-amber-200':['failed','void','terminated','cancelled'].includes(v)?'bg-rose-50 text-rose-200 border-rose-200':'bg-slate-800 text-slate-700 border-slate-300';return <span className={`inline-block rounded border px-2 py-1 text-[10px] uppercase ${cls}`}>{value}</span>}
