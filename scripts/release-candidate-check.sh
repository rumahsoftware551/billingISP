...
grep -q '"product_version"[[:space:]]*:[[:space:]]*"1.3.0-rc4"' PHASE.json || fail "PHASE.json product_version mismatch"
...
grep -q '^JARINGANKU_VERSION=1.3.0-rc4$' .env.production.example || fail "JARINGANKU_VERSION mismatch"
